...
async function fetchOHLC(symbol, interval, range) {
  ...
  const r = await axios.get(url, { timeout: 10000, httpsAgent: agent, headers: { 'User-Agent': 'Mozilla/5.0' } });
...
async function getLevels(symbol) {
  try {
    const [daily, weekly, monthly] = await Promise.all([
      fetchOHLC(symbol, '1d', '3mo'),
      fetchOHLC(symbol, '1wk', '1y'),
    ...
    return {
      ...
      daily: { ...calcPivots(prevDay.high, prevDay.low, prevDay.close), ...calcEma7(daily.candles) },
      weekly: { ...calcPivots(wc.high, wc.low, wc.close), ...calcEma7(weekly.candles) },
      monthly: { ...calcPivots(mc.high, mc.low, mc.close), ...calcEma7(monthly.candles) },
    };
  } catch { return null; }
}
